...
public class HangmanGameService extends GameService {
    ...
    public HangmanGameService(Question question) {
        super(question);
        availableLetters = SpecificPropertiesUtils.getText(TournamentGame.getInstance().getAppInfoService().getLanguage() + "_hangman_available_letters");
    ...
    public void processNewGameQuestionInfo(GameUser gameUser, GameQuestionInfo gameQuestionInfo) {
        ...
        if (TournamentStage.getQuestionDifficultyLevelToPressFirstAndLastLetter().contains(question.getQuestionDifficultyLevel())) {
    ...
    private String normalize(String answer) {
        return StringUtils.stripAccents(answer);
//        Normalizer.normalize(answer1, Normalizer.Form.NFD).replaceAll("\\p{InCombiningDiacriticalMarks}+", "");
    }
